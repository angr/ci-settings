...
def typecheck_files(filenames):
    ...
    my_report = {os.path.realpath(filename): FileReport(filename, diagnostics=[]) for filename in filenames}
...
def typecheck_diff(rev1, rev2):
    ...
    for filename in filenames:
        ...
        if filename in report1:
            base_errors = report1[filename].errors
        else:
            base_errors = 0
        e2 = report2[filename]
        if e2.errors > base_errors:
            status += 1
            print(f"### {filename} errors increased from {base_errors} to {e2.errors}. Please fix:")
            for line, char, severity, text in sorted(e2.diagnostics):
                print(f"{filename}:{line}:{char}: [{severity}] {text}")
        elif e2.errors < base_errors:
            print(f"### {filename} errors decreased from {base_errors} to {e2.errors}. Nice!")
        else:
            print(f"### {filename} errors remained at {base_errors}. Nice!")
